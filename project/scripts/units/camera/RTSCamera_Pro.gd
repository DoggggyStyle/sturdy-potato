...
# Godot 4.x 版：RTS 相机（WASD/边缘滚屏/滚轮变焦/QE 旋转/中键拖拽/双击居中/跟随）
...
@export var ground_mask: int = 1           # 地面碰撞层
...
func _ready() -> void:
	...
	if cam == null:
		cam = $Camera3D if has_node("Camera3D") else null
...
func _unhandled_input(e: InputEvent) -> void:
	if e is InputEventMouseButton:
		if e.button_index == MOUSE_BUTTON_WHEEL_UP and e.pressed:
			_target_dist = maxf(zoom_min, _target_dist - zoom_step)
			if Input.is_key_pressed(KEY_SHIFT): _lock_current_tilt()
		elif e.button_index == MOUSE_BUTTON_WHEEL_DOWN and e.pressed:
			_target_dist = minf(zoom_max, _target_dist + zoom_step)
			if Input.is_key_pressed(KEY_SHIFT): _lock_current_tilt()
			...
			if now - _last_rclick_ms <= double_click_ms:
				_center_to_mouse_ground()
			_last_rclick_ms = now
...
func start_follow(target: Node3D, offset := Vector3.ZERO) -> void:
	...
	if _follow_target:
		emit_signal("follow_started", _follow_target)
